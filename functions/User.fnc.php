...
function User( $item )
{
	...
	if ( ! isset( $_ROSARIO['User'] )
	        || empty($_ROSARIO['User'][1]['SYEAR']) || UserSyear() !== $_ROSARIO['User'][1]['SYEAR'] )
	{
		// Get User Info.
...
function Preferences( $item, $program = 'Preferences' )
{
	...
	{
	        $_ROSARIO['Preferences'][ $program ][ $item ][1]['VALUE'] = isset($defaults[ $item ]) ? $defaults[ $item ] : '';
	}
